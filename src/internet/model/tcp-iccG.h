 ...
 *      Author: root
 *
 *  ICC-G: ICC congestion control with the FFTW-based dominant-frequency
 *  detection in find_fm() replaced by an online Goertzel algorithm.
 */
...
class TcpIccG : public TcpNewReno
{
public:
  /**
   ...
   */
  TcpIccG (const TcpIccG& sock);
  virtual ~TcpIccG (void);
  ...
  // ── Goertzel online dominant-frequency detection ──────────────────
  void GoertzelInit(double fs, int N);
  void GoertzelFeed(double qd_sample, double cwnd_sample);
...
private:
  ...
  // ── Goertzel online state ──────────────────────────────────────────
  // Candidate frequency bins monitored in parallel.
  // g_freqs[k]       : candidate frequency value (Hz)
  // g_s1[k], g_s2[k] : Goertzel IIR state for QdArray signal
  // g_c1[k], g_c2[k] : Goertzel IIR state for CwndArray signal
  // g_coeff[k]       : 2*cos(2π*f_k/fs), precomputed when fs is known
  // g_n              : number of samples ingested since last reset
  // g_fs             : current sampling rate (samples/s), updated each cycle
  // g_N              : window length used to normalise power (= samples in cycle)
  static constexpr int G_BINS = 64;   // number of candidate bins
  double g_freqs[G_BINS];
  double g_s1[G_BINS],  g_s2[G_BINS];   // RTT/Qd signal state
  double g_c1[G_BINS],  g_c2[G_BINS];   // cwnd signal state
  double g_coeff[G_BINS];
  int    g_n;
  double g_fs;
  int    g_N;
  bool   g_initialized;
  double g_dcQd, g_dcCw;   // DC estimate subtracted at feed time (prev-cycle means)
};
